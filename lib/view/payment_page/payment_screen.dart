...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  String _generateUpiWhatsAppMessage() {
    final paymentData = _controller.paymentDetails.value.data;
    final amount = paymentData?.totalAmount ?? "0";
    final monthName = _getMonthNameFromNumber(selectedMonth).tr;
    final year = DateTime.now().year;
    final customerId = Constant.userId.value;
    final milkQuantity = paymentData?.milkQuantity ?? "0";
    final totalLiters = paymentData?.totalLitre ?? "0";

    return "નમસ્તે, મેં મારું દૂધ પેમેન્ટ UPI દ્વારા કરી દીધી છે.\n\n"
        "કસ્ટમર આઈડી: $customerId\n"
        "મહિનો: $monthName $year\n"
        "દૂધનું પ્રમાણ: $milkQuantity લિટર\n"
        "કુલ લિટર: $totalLiters લિટર\n"
        "કુલ રકમ: ₹$amount\n"
        "પેમેન્ટ મેથડ: UPI\n\n"
        "કૃપા કરીને પેમેન્ટ ચેક કરો અને કન્ફર્મ કરો.";
  }

  String _generateCashWhatsAppMessage() {
    final paymentData = _controller.paymentDetails.value.data;
    final amount = paymentData?.totalAmount ?? "0";
    final monthName = _getMonthNameFromNumber(selectedMonth).tr;
    final year = DateTime.now().year;
    ...
    final milkQuantity = paymentData?.milkQuantity ?? "0";
    final totalLiters = paymentData?.totalLitre ?? "0";
    final receiverName = selectedPerson ?? "";

    return "નમસ્તે, મેં મારું દૂધ પેમેન્ટ Cash દ્વારા કરી દીધી છે.\n\n"
        "કસ્ટમર આઈડી: $customerId\n"
        "મહિનો: $monthName $year\n"
        "દૂધનું પ્રમાણ: $milkQuantity લિટર\n"
        "કુલ લિટર: $totalLiters લિટર\n"
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  bool isImageUploaded = false;
  bool isUploading = false;
  bool _paymentConfirmed = false; // New checkbox state
  final ApiController _controller = Get.put(ApiController());
  final ImagePicker _imagePicker = ImagePicker();
  bool _showFilters = false;

  // WhatsApp number
  final String _whatsappNumber = "9328367662";
  ...
  // Generate WhatsApp message in Gujarati for UPI
  String _generateUpiWhatsAppMessage() {
    final paymentData = _controller.paymentDetails.value.data;
    final amount = paymentData?.totalAmount ?? "0";
    final monthName = _getMonthNameFromNumber(selectedMonth).tr;
  ...
  // Generate WhatsApp message in Gujarati for Cash
  String _generateCashWhatsAppMessage() {
    final paymentData = _controller.paymentDetails.value.data;
    final amount = paymentData?.totalAmount ?? "0";
    final monthName = _getMonthNameFromNumber(selectedMonth).tr;
  ...
  void _submitPayment() {
    ...
    if (selectedPayment == "upi".tr) {
      // Case 1: UPI selected but payment not confirmed
      if (!_paymentConfirmed) {
        Get.to(
          UpiPaymentScreen(
            amount: amount.toString(),
      ...
      // Case 2: UPI selected and payment confirmed
      _showUpiConfirmationDialog();
      return;
    }
  ...
  Future<void> _processUpiPayment() async {
    ...
    final body = {
      ...
      "year": DateTime.now().year,
      "totalAmount": amount,
      "image": _generateCashWhatsAppMessage,
      // Empty since we're not uploading image anymore
    };
    print("body :: $body");
  ...
  void _processCashPayment() async {
    ...
    // Send WhatsApp message for Cash
            ...
            child: Column(
              ...
              children: [
                ...
                if (_controller.paymentDetails.value.data?.paymentStatus ==
                  ...
                  // ✅ UPI Payment Confirmation Section (REPLACED Image Upload)
                  if (selectedPayment == "upi".tr)
                    _buildUpiPaymentConfirmation(),
  ...
  // REPLACED: UPI Image Upload with Payment Confirmation Checkbox
  Widget _buildUpiPaymentConfirmation() {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
      ...
      child: Column(
        ...
        children: [
          ...
          if (_paymentConfirmed) ...[
            ...
            // WhatsApp Message Info for Cash
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
                color: Colors.blue.shade50,
  ...
  // Cash Person Selection with WhatsApp Info
  Widget _buildCashPersonSelection() {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
  ...
  // ... (Keep all other existing widget methods exactly as they were)
  // _buildHeaderSection, _buildFilterSection, _buildSummaryCard, etc. remain unchanged
      ...
      child: RadioListTile<String>(
        ...
        onChanged: (value) {
          setState(() {
            ...
            if (selectedPayment == "upi".tr) {
              selectedPerson = null;
              _paymentConfirmed = false; // Reset checkbox
            } else {
              _paymentConfirmed = false; // Reset checkbox
            }
          });
