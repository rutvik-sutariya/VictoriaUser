...
class HomeScreen extends StatelessWidget {
  ...
  Widget _buildDeliveryCards() {
    final morningLiters = _controller.userDetails.value.data?.morningLiters ?? 0;
    final eveningLiters = _controller.userDetails.value.data?.eveningLiters ?? 0;

    final hasMorning = morningLiters != 0;
    final hasEvening = eveningLiters != 0;

    // If no deliveries scheduled
    ...
    return Row(
      children: [
        ...
        // Add spacing only if both cards are shown
        if (hasMorning && hasEvening) const SizedBox(width: 16),
        ...
        children: [
          ...
          Expanded(
            child: _buildActionButton(
              ...
              onTap: () {
                _activeCard.value =
                _activeCard.value == 'reduced' ? '' : 'reduced';
              },
...
class OrderExtraMilkCard extends StatelessWidget {
            ...
            if (hasMorning)
              Expanded(
                child: _buildSlotButton(
                    "Morning", Icons.wb_twilight_rounded, hasMorning, hasEvening),
              ),
  ...
  Widget _buildSlotButton(String slot, IconData icon, bool hasMorning,
      bool hasEvening) {
    final isSelected = extraOrder.value == slot;
    final isSingleSlot = (hasMorning && !hasEvening) || (!hasMorning && hasEvening);
...
class NoOrderMilkCard extends StatelessWidget {
  ...
  Widget _buildCancelSlotButton(String slot, IconData icon, bool hasMorning,
      bool hasEvening) {
...
class HomeScreen extends StatelessWidget {
  ...
  Widget _buildDeliveryCards() {
    return Row(
      children: [
        if (_controller.userDetails.value.data?.morningLiters != 0)
          Expanded(
            child: DeliveryCard(
              title: "morning".tr,
              liter:
        ...
        const SizedBox(width: 16),
        if (_controller.userDetails.value.data?.eveningLiters != 0)
          Expanded(
            child: DeliveryCard(
              title: "evening".tr,
              liter:
        ...
        children: [
          ...
          Expanded(
            child: _buildActionButton(
              ...
              color: Colors.orange,
              onTap: () {
                _activeCard.value = _activeCard.value == 'reduced'
                    ? ''
                    : 'reduced';
              },
              isActive: _activeCard.value == 'reduced',
          ...
          Expanded(
            child: _buildActionButton(
              ...
              color: Colors.red,
              onTap: () {
                _activeCard.value = _activeCard.value == 'cancel'
                    ? ''
                    : 'cancel';
              },
              isActive: _activeCard.value == 'cancel',
